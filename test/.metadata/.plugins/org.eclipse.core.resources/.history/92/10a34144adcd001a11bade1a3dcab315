...
public class Machine implements IMachine {
					...
					if (checkMoreString.equals("y")) {
						...
						for (int i = 0; i < product.length; i++) {
							if (orderProduct.get(product[i]) > 0) {
								...
								count[i] = orderProduct.get(product[i]);
								
							}
						}
						recept(product, count, Integer.toString(money));
						checkMore = false;
					}
	...
	@Override
	public void recept(String[] product, int[] count, String chance) {
		/*
		...
		try {
			...
			for (int i = 0; i < count.length; i++) {
				if (count[i]> 0) {
					buffer.append("your order: " + product[i] + "\r");
					buffer.append(product[i] + "-count: " + count[i] + "\r");
				}
			}
